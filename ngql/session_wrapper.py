...
class NebulaQueryError(NebulaError):
    pass


class NebulaSessionWrapper:
    _session: Session
    ...
    def from_conn_pool(
        ...
        return cls(
            conn_pool.get_session(user_name=user_name, password=password),
            space,
        )
    ...
    def space(self):
        return self._session.execute(
            f"SHOW SESSION {self._session._session_id};"
        ).space_name()

    def _query(self, query: str) -> ResultSet:
        """Excute query and return ResultSet.

        Raises:
            NebulaQueryError: If query failed.
            NebulaError: If any other error occurs.
        """
        query = query.replace("\\\n", "\n")
        try:
            result = self._session.execute(query)
            if not result.is_succeeded():
                raise NebulaQueryError(result.error_msg())
        except Exception as e:
            raise NebulaError(f"[ERROR]:\n { e }")
        finally:
            return result

    def ng_load(self, args: LoadDataArgsModel):
        """
        ...
        r = self._query(f"DESC SPACE `{space}`")
        try:
        ...
        r = self._query(f"DESCRIBE {DESC_TYPE} `{DESC_TARGET}`")
        ...
        if args.tag:
            ...
            for i in tqdm(
                ...
                try:
                    ...
                    raise Exception(
                        f"INSERT Failed on row {i + index}, data: {row}"
                    ) from e
...
"""
    client = NebulaSessionWrapper.from_conn_pool(
        conn_pool=conn_pool,
        user_name="root",
        password="nebula",
    )
